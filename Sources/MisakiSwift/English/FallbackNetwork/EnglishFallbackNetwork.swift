...
final class EnglishFallbackNetwork {
  ...
  /// Locate a bundled resource, preferring the `Resources/` subdirectory
  /// (upstream layout) but falling back to bundle root so consumers that
  /// flatten the resource bundle to satisfy iOS codesign still resolve.
  private static func bundledResourceURL(named name: String, ext: String) -> URL? {
  ...
  private static func loadConfig(british: Bool) -> BARTConfig? {
    ...
    guard let url = bundledResourceURL(named: fileName, ext: "json"),
  ...
  private static func loadWeights(british: Bool) -> [String: MLXArray]? {
    ...
    guard let url = bundledResourceURL(named: fileName, ext: "safetensors"),
